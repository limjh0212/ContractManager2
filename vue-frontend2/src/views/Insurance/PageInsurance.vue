        ...
        <v-card class="card shadow" style="width: 750px; margin: 20px; padding: 60px 100px">
            ...
            <form class="row g-3" style="text-align: left;">
                ...
                <v-row>
                    <div class="col-md-4">
                        ...
                        <input id="inpustDiposit" v-model="cost" class="form-control" name="cost"
                               readonly style="text-align:right;" type="text">
                    </div>&nbsp;
                    <div class="col-md-4">
                        <label class="form-label" for="inputPrecost">전년 비용</label>
                        <input id="inputPrecost" v-model="precost" class="form-control" name="precost"
                               readonly style="text-align:right;" type="text">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label" for="inputDiffcost">전년 대비</label>
                        <input id="inputDiffcost" v-model="diffcost" class="form-control" name="precost"
                               readonly style="text-align:right;" type="text">
                    </div>
                </v-row>
                <v-row>
                    <div class="col-md-4">
                        <label class="form-label" for="client">보험사</label>
                        <input id="client" v-model="item.client" class="form-control"
                               name="client" readonly type="text">
                    </div>&nbsp;
                    <div class="col-md-4">
                        <label class="form-label" for="clientNumber">보험사 연락처</label>
            ...
            <div style="margin-top: 20px">
                <table v-for="(item, idx) in cmnt1" class="table" style="font-size: 15px; margin-bottom:0px">
                    <tr>
                        <td style="width:60px; height : 35px; text-align: left">
                            {{ item.user ? item.user.nickname : '' }}
                        </td>
                        <td style="width: 350px">{{ item.content }}</td>
                        ...
                        >
                            ...
                            <button v-if="isLoginUser(item.user?item.user.id:'')" v-show="item.content !== deleteCmntMessage"
                    ...
                    <tr v-for="(item, i) in cmnt2[idx]">
                        ...
                        <td style="width: 350px; "><i class="material-icons" style="font-size: 13px;">subdirectory_arrow_right</i>{{ item.content }}</td>
                        <td v-if="$moment().format('YYYY-MM-DD') === $moment(item.created).format('YYYY-MM-DD')"
                            style="width: 100px; text-align: center">
                            {{ $moment(item.created).format('HH:mm:ss') }}
                        </td>
...
import {api_deleteInsurance, api_getOneInsurance} from "@/api/api_insurance";
...
export default {
    ...
    methods: {
        async FETCH_DATA() {
            const {data} = await api_getOneInsurance(this.$route.params.id, this.$store.state.id)
            this.item = data.insurance;
            this.noticeUser = data.notice;
            this.noticeid = data.noticeid;
            this.cmnt1 = data.cmnt1;
            this.cmnt2 = data.cmnt2;
            this.startdate = convertDate(data.insurance.startdate);
            this.enddate = convertDate(data.insurance.enddate);
            this.cost = data.insurance.cost.toLocaleString() + "원";
            this.precost = data.insurance.precost.toLocaleString() + "원";
            this.diffcost = data.insurance.diffcost.toLocaleString() + "원";
        },
        async saveNotice() {
            const Data = {
                section  : 2,
        ...
        async sendNotice() {
            await api_sendMail(2, this.$route.params.id)
            alert('알림을 발송했습니다.')
        },
        isLoginUser(id) {
        ...
        goEdit() {
            this.$router.push(`/insurance/edit/${this.item.id}`);
        },
        async deleteItem() {
            //TODO: 삭제 확인 창
            await api_deleteInsurance(this.item.id);
            await this.$router.push('/insurance');
        },
        saveCmnt1_Btn() {
