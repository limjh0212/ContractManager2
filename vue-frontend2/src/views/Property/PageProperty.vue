            ...
            <form class="row g-3" style="text-align: left;">
                <v-row>
                    <div class="col-md-3">
                        ...
                        <input id="inpustDivision" v-model="item.division" class="form-control" name="division"
                               readonly type="text">
                    </div>
                </v-row>
                <v-row>
                ...
                <v-row>
                    <div class="col-md-4">
                        <label class="form-label" for="inpustDiposit">보증금</label>
                        <input id="inpustDiposit" v-model="deposit" class="form-control" name="diposit"
                               readonly style="text-align:right;" type="text">
                    </div>&nbsp;
                    <div class="col-md-4">
                        <label class="form-label" for="inpustmonthlyFee">월 임차료</label>
                ...
                <v-row>
                    <div class="col-md-4">
                        <label class="form-label" for="inpustDiposit">임대인</label>
                        <input id="client" v-model="item.client" class="form-control"
                               name="client" readonly type="text">
                    </div>&nbsp;
                    <div class="col-md-4">
                        <label class="form-label" for="inpustmonthlyFee">임대인 연락처</label>
            ...
            <div style="margin-top: 20px">
                <table v-for="(item, idx) in cmnt1" class="table" style="font-size: 15px; margin-bottom:0px">
                    <tr>
                        ...
                        <td style="width: 350px; padding: 5px 0px">{{ item.content }}</td>
                        ...
                        <td style="width: 90px; height: 40px; text-align: right; vertical-align: center"
                        >
...
<script>
import {api_deleteProperty, api_getOneProperty} from "@/api/api_property";
...
export default {
    ...
    methods: {
        async FETCH_DATA() {
            const {data} = await api_getOneProperty(this.$route.params.id, this.$store.state.id)
            this.data = data
            this.item = data.property;
            this.noticeUser = data.notice;
            this.noticeid = data.noticeid;
            this.cmnt1 = data.cmnt1;
            this.cmnt2 = data.cmnt2;
            this.startdate = convertDate(data.property.startdate);
            this.enddate = convertDate(data.property.enddate);
            this.deposit = data.property.deposit.toLocaleString() + "원";
            this.monthlyfee = data.property.monthlyfee.toLocaleString() + "원";
        },
        async saveNotice() {
            const Data = {
                section  : 1,
        ...
        async sendNotice() {
            await api_sendMail(1, this.$route.params.id)
            alert('알림을 발송했습니다.')
        },
        isLoginUser(id) {
        ...
        goEdit() {
            this.$router.push(`/property/edit/${this.item.id}`);
        },
        async deleteItem() {
            //TODO: 삭제 확인 창
            await api_deleteProperty(this.item.id);
