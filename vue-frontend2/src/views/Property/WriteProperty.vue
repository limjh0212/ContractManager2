        ...
        <div class="card shadow" style="width: 750px; margin: 30px; padding: 60px 100px">
            ...
            <form class="row g-3" style="text-align: left;">
                ...
                <v-row style="margin: 30px 0px">
                    ...
                    <!--    <button class="btn btn-outline-secondary" type="button" id="inputGroupFileAddon04" @onclick="">-->
                    <!--        Upload-->
                    <!--    </button>-->
                    <!--</div>-->
                </v-row>
...
import {api_getEditProperty, api_saveProperty} from "@/api/api_property";
...
export default {
    ...
    methods: {
        async FETCH_DATA() {
            if (this.$route.params.id !== null) {
                const {data} = await api_getEditProperty(this.$route.params.id)
                this.id = data.id;
    ...
    created() {
        if (this.$route.path !== '/property/write/') {
            this.FETCH_DATA();
        }
    }
}
